...
installLogic() {
	...
	if [[ "${installResult[*]}" =~ "was not found in the DARE Update App catalog." ]]; then
		...
		exit 1
	elif [[ "${installResult[*]}" =~ "Error:" ]]; then
		if [[ "${installResult[*]}" =~ "install was successful" ]]; then
			/bin/echo "The $targetApp install was successful."
			/bin/echo ""
			/bin/echo "######### Install Output: #########"
			/bin/echo ""
			runOutput=$(printf "%s\n" "${installResult[@]}" | /usr/bin/sed 's/\[;m//g')
			/bin/echo "$runOutput"
			exit 0
		else
	...
	elif [[ "${installResult[*]}" =~ "install was successful" ]]; then
		/bin/echo "The $targetApp install was successful"
		/bin/echo ""
		/bin/echo "######### Install Output: #########"
		/bin/echo ""
