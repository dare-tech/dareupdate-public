...
# Parameter 6 - Custom recurring cached update check interval in minutes. The default if no custom value is defined is 15 minutes. To disable user prompts, use "disable" as the value (Optional: Do not modify unless necessary)
# Parameter 7 - Custom recurring forced update check interval in minutes. The default if no custom value is defined is 60 minutes. To disable user prompts, use "disable" as the value (Optional: Do not modify unless necessary)
#
###########################################################################################################################################################################################################
###########################################################################################################################################################################################################
#
...
if [[ $(/bin/echo "$scheduledCheckIntervalMinutes" | /usr/bin/awk '{print tolower($0)}') != "disable" ]]; then
	...
	/bin/echo "Recurring update prompts were disabled. Skipping creating the scheduled update daemon."
fi
if [[ $(/bin/echo "$forcedCheckIntervalMinutes" | /usr/bin/awk '{print tolower($0)}') != "disable" ]]; then
	...
	/bin/echo "Forced update prompts were disabled. Skipping creating the forced update daemon."
fi
